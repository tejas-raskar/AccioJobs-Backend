...
def clean_jobs(job, key_mappings):
    ...
    for key, api_key in key_mappings.items():
        if key == "description":
            parsed_description = BeautifulSoup(job.get(api_key), 'html_parser')
...
remoteok_response = requests.get("https://remotive.com/api/remote-jobs")
